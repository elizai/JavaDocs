...
for(String cheie : countries.keySet ())
    if(cheie.startsWith("R"))
        seekingCountries.add(cheie);
    ...
    public List<String> iteratingOverEntries() {
        ...
        List<String> seekingCountries = new ArrayList<String>();
        for (Map.Entry<String, String> pereche : countries.entrySet())
            if (pereche.getKey().contains("Q") || pereche.getKey().contains("q"))
                seekingCountries.add(pereche.getKey().toLowerCase());
